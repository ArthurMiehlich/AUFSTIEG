...
def get_random_instance(N_nodes, randomized=True):
    ...
    missions = {'Depot_s': [(0, 0), 0], 'Depot_t': [(0, 0), 0]}
    if randomized:
...
if __name__ == "__main__":
    ...
    missions2 = {'Depot_s': [(0, 0), 0], 'Depot_t': [(0, 0), 0], 'A': [(1, 4), 2], 'B': [(2, 3,), 2], 'C': [(4, 3), 2],
                 'D': [(3, -3), 2], 'E': [(1, -3), 2], 'F': [(-3, -1), 2], 'G': [(-4, 0), 2]}

    missions=get_random_instance(30,False)

    # [velocity, max energy]
    drones = {'quadro1': [5, 30], 'quadro2': [5, 30], 'quadro3': [5, 30]}  # ,'quadro4':[5,30]}

    create_graph(missions2)
